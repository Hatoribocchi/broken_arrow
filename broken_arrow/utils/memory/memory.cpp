...
void* MEM::GetModuleBaseHandle( const wchar_t* wszModuleName )
{
	...
	if ( pModuleBase == nullptr )
		L_PRINT( LOG_ERROR ) << XOR_STR( "module base not found: \"" ) << wszModuleName << XOR_STR( "\"" );
...
void* MEM::GetExportAddress( const void* hModuleBase, const char* szProcedureName )
{
	...
	{
		L_PRINT( LOG_ERROR ) << XOR_STR( "module has no exports: \"" ) << GetModuleBaseFileName( hModuleBase ) << XOR_STR( "\"" );
		return nullptr;
	}
...
#ifdef _DEBUG
			L_PRINT( LOG_INFO ) << XOR_STR( "export found: \"" ) << reinterpret_cast< const char* >( pBaseAddress + pNamesRVA[ uMiddle ] ) << XOR_STR( "\" in \"" ) << GetModuleBaseFileName( hModuleBase ) << XOR_STR( "\" at: " ) << L::AddFlags( LOG_MODE_INT_SHOWBASE | LOG_MODE_INT_FORMAT_HEX ) << uFunctionRVA;
#else
			L_PRINT( LOG_INFO ) << XOR_STR( "export found: " ) << szProcedureName;
#endif // _DEBUG
	...
	L_PRINT( LOG_ERROR ) << XOR_STR( "export not found: " ) << szProcedureName;
...
bool MEM::GetSectionInfo( const void* hModuleBase, const char* szSectionName, std::uint8_t** ppSectionStart, std::size_t* pnSectionSize )
{
	...
	L_PRINT( LOG_ERROR ) << XOR_STR( "code section not found: \"" ) << szSectionName << XOR_STR( "\"" );
	return false;
}
...
std::uint8_t* MEM::FindPattern( const wchar_t* wszModuleName, const char* szBytePattern, const char* szByteMask )
{
	...
	if ( hModuleBase == nullptr )
	{
		L_PRINT( LOG_ERROR ) << XOR_STR( "failed to get module handle for: \"" ) << wszModuleName << XOR_STR( "\"" );
		return nullptr;
	}
	...
	if ( pIDH->e_magic != IMAGE_DOS_SIGNATURE )
	{
		L_PRINT( LOG_ERROR ) << XOR_STR( "failed to get module size, image is invalid" );
		return nullptr;
	}
...
#ifdef PARANOID_PATTERN_UNIQUENESS
	...
	if ( !vecFoundOccurrences.empty( ) )
	{
		...
		if ( vecFoundOccurrences.size( ) > 1U )
		{
			...
			L_PRINT( LOG_WARNING ) << XOR_STR( "found more than one occurrence with \"" ) << szPattern << XOR_STR( "\" pattern, consider updating it!" );
	...
	if ( pFoundAddress == nullptr )
	{
		...
		L_PRINT( LOG_ERROR ) << XOR_STR( "pattern not found: \"" ) << szPattern << XOR_STR( "\"" );
