...
#define L_PRINT(LEVEL) L::stream(LEVEL, "[" __FILE_NAME__ ":" STRINGIFY(__LINE__) "] ")
#else
#define L_PRINT(LEVEL) L::stream(LEVEL, L::DETAIL::MakeFileBlock<CRT::StringLength(L::DETAIL::GetFileName(__FILE__)), CRT::StringLength(STRINGIFY(__LINE__))>(L::DETAIL::GetFileName(__FILE__), STRINGIFY(__LINE__)).Get())
#endif
#else
